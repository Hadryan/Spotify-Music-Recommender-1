...
class SpotifyRecommender:
    ...
    def predict(self, idx: int, features_df: pd.DataFrame) -> Union[str, pd.DataFrame]:
        artists = []
        track_names = []
        track_ids = []
        try:
    ...
    def get_info(self, track_id: str) -> str:
        urn = f"spotify:track:{track_id}"
        track = self.sp.track(urn)
        thumbnail_url = track["album"]["images"][0]["url"]
        return thumbnail_url


    def visualize_songs(self, out_df: pd.DataFrame) -> None:
        """
        ...
        for idx, row in out_df.iterrows():
            image_urls.append(self.get_info(row["track_id"]))
